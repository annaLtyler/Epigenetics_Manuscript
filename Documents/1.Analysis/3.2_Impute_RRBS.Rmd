...
if(start.at < length(methyl.id)){
  ...
  for(tr in start.at:length(perm.rrbs.lod)){
      ...
      if(tr %% save.every == 0){
          saveRDS(perm.rrbs.lod, perm.rrbs.lod.file)
          saveRDS(perm.rrbs.r2, perm.rrbs.r2.file)
      }
...
```{r assess_perm}
max.obs.lod <- sapply(all.rrbs.lod, function(x) if(length(x) > 1){max(x, na.rm = TRUE)}else{NA})
max.null.lod <- apply(perm.rrbs.lod, 1, function(x) if(!all(is.na(x))){max(x, na.rm = TRUE)}else{NA})

plot(max.null.lod, max.obs.lod, xlab = "Effect of Haplotype Alone",
  ylab = "Observed LOD")
abline(0,1)
```