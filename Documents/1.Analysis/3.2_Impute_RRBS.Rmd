...
if(females_only){
  rrbs.r2.file <- gsub(".RDS", "_females.RDS", rrbs.r2.file)
  rrbs.coef.file <- gsub(".RDS", "_females.RDS", rrbs.coef.file)

}
if(chow_only){
  rrbs.r2.file <- gsub(".RDS", "_chow.RDS", rrbs.r2.file)
  rrbs.coef.file <- gsub(".RDS", "_chow.RDS", rrbs.coef.file)
}
...
if(!file.exists(rrbs.r2.file) || delete_previous){
    ...
    for(i in 1:length(all.rrbs.r2)){
        ...
        common.ind <- intersect(rownames(adj.expr), rownames(genoprobs[[1]]))
...
imputed.effect.file <- here("Results", "RRBS", "Imputed_RRBS_effect.RDS")
if(females_only){
  imputed.effect.file <- gsub(".RDS", "_females.RDS", imputed.effect.file)
}
if(chow_only){
  imputed.effect.file <- gsub(".RDS", "_chow.RDS", imputed.effect.file)
}
saveRDS(imp.rrbs.effect, imputed.effect.file)
...
scan_one_rrbs <- function(perm.data, p){
  ...
  #plot(as.numeric(rownames(all.rrbs.lod[[i]])), all.rrbs.lod[[i]][,1], type  = "h",xlab = "Genomic Position", ylab = "DNA Methylation LOD score")

  common.ind <- intersect(rownames(perm.data$gene.expr), rownames(imp.geno[[1]]))
  common.expr.locale <- match(common.ind, rownames(perm.data$gene.expr))
  common.genoprob.locale <- match(common.ind, rownames(imp.geno[[1]]))
...
if(females_only){
  perm.lod.rrbs.file <- gsub(".RDS", "_females.RDS", perm.lod.rrbs.file)
  perm.r2.rrbs.file <- gsub(".RDS", "_females.RDS", perm.r2.rrbs.file)
}
if(chow_only){
  perm.lod.rrbs.file <- gsub(".RDS", "_chow.RDS", perm.lod.rrbs.file)
  perm.r2.rrbs.file <- gsub(".RDS", "_chow.RDS", perm.r2.rrbs.file)
}
...
if(start.at < length(methyl.id)){
  ...
  for(tr in start.at:length(perm.rrbs.lod)){
      ...
      perm.data <- list("gene.expr" = gene.expr[sub.idx,,drop=FALSE], 
...
perm.results.file <- here("Results", "RRBS", "Perm_results.RDS")
