...
## Permute RRBS LOD and R2

We need to do permutations on these as we did for chromatin state.
We permute the haplotype that is assigned to each DNA methylation
vector.
            ...
            if(!is.na(rrbs.array)){
              ...
              if(length(gene.expr) > 0){
                ...
                perm.data <- list("gene.expr" = gene.expr, 
                  ...
                  #check for NAs
                  if(all(sapply(perm.rrbs.results, length) > 1)){
              ...
              } #end case for existence of gene expression data
            } #end case for existence of RRBS data
        } #end case for chromosome check
      } #end check for size of methylation matrix
...
We look at the distribution of p values across all transcripts.
If it is skewed toward 0, there is a signal here. 
If it is totally uniform, the 
...
max.rrbs.lod <- sapply(all.rrbs.lod, function(x) if(length(x) > 1){max(x[,1])}else{NA})
rrbs.p <- sapply(1:length(all.rrbs.lod), 
  function(x) if(!all(is.na(perm.rrbs.lod[x,]))){emp.p(max.rrbs.lod[x], perm.rrbs.lod[x,])}else{NA})
names(rrbs.p) <- names(all.rrbs.lod)

rrbs.p[low.lod.idx] <- NA
hist(rrbs.p, breaks = 100)

examples <- which(rrbs.p == 0)
i <- examples[4]
gene.name = gene.info[which(gene.info[,"ensembl_gene_id"] == names(rrbs.p)[i]),"external_gene_name"][1]
...
```{r assess_perm}
geno.r2 <- readRDS(here("Results", "ChromHMM", "14_states_C", "eQTL.r2.RDS"))
...
names(r2.p) <- names(all.rrbs.lod)


hist(r2.p, breaks = 100)

plot(-log10(r2.p+1e-2), max.rrbs.r2)
