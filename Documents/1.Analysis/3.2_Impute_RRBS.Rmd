...
if(start.at < length(methyl.id)){
  ...
  for(tr in 1:length(perm.rrbs.lod)){
      print(tr)
      methyl.mat.locale <- which(names(methyl.mats) == ordered.transcript.info[tr,"external_gene_name"])

      #checks for methylation matrix
      if(length(methyl.mats[[methyl.mat.locale]]) <= 1){message("\tCannot locate array");next()} #it exists
      if(ncol(methyl.mats[[methyl.mat.locale]]) > 2000){message("\tArray too large");next()} #is is of a reasonable size

      transcript.chr <- ordered.transcript.info[tr,"chromosome_name"]

      #checks for chromosome
      if(is.na(transcript.chr) || transcript.chr == "X" || transcript.chr == "MT"){message("\tTranscript not on autosome");next()}
      ...
      if(is.na(rrbs.array)){message("\tNo RRBS array"); next()}
      ...
      #check for gene expr
      if(length(expr.locale) == 0){message("\tCannot find expression");next()}
      ...
      if(!all(sapply(perm.rrbs.results, length) > 1)){message("\t No data returned");next()}
