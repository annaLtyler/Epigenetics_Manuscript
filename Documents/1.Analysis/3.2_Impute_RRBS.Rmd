...
if(start.at < length(methyl.id)){
  ...
  for(tr in 6097:length(perm.rrbs.lod)){
      ...
      print(paste("transcript chr:", transcript.chr))
      chr.choice <- transcript.chr != "X"
      print(paste("Chr choice:", chr.choice))
      ...
      print(paste("Gene expression:", length(expr.locale)))
      print(paste("choice:", as.character(gene.expr.choice)))

      #check for gene expr
      if(length(expr.locale) == 0){next()}
      gene.expr <- expr[,expr.locale,drop=FALSE]

      adj.expr <- adjust(gene.expr, covar)
      
      print("made it through the adjustment")
