...
imageWithText(all_state_methyl_levels, col.scale = "blue", grad.dir = "high")
boxplot(all_state_methyl_levels)

intragenic.idx <- intersect(which(coords >= 0), which(coords <= 1))
barplot_with_num(sort(round(rowMeans(all_state_methyl_levels[,intragenic.idx]))))

methyl.min <- apply(all_state_methyl_levels, 2, function(x) min(x, na.rm = TRUE))
methyl.max <- apply(all_state_methyl_levels, 2, function(x) max(x, na.rm = TRUE))
plot.poly.xy(coords, methyl.min, coords, methyl.max, new.plot = TRUE)
```
...
if(is.interactive){quartz(width = 10, height = 7)}
par(mfrow = c(1,1))
full.list <- lapply(1:ncol(full_strain_mat), function(x) full_strain_mat[row.has.val,x])
plot_mean_conf(coords, full.list, spar = 0.4,
    xlab = "Relative Position", ylab = "Methylation Percent Mean", 
    main = "All methylation across all states", ylim = c(0, 100))
...
for(state in 1:num.states){
    ...
    strain_methyl_effect <- lapply(1:length(all.strain.methyl), 
        ...
        function(x) if(length(unique(all.strain.methyl[[y]][,x])) > 1){plot.with.model(rankZ(all.strain.methyl[[y]][,x]), all.strain.expr[[y]], 
        confidence = 0.95, plot.results = FALSE)}else{rep(NA, 5)}))
    #pheatmap(strain_methyl_effect[[1]], cluster_cols = FALSE, cluster_rows = FALSE)
...
methyl_bin_file <- here("Results", "RRBS", "Methylation_presence_by_state.RDS")
...
for(state in 1:num.states){
    ...
    #also look at presence/absence of a CpG site
    bin.methyl <- overall_methyl
    bin.methyl[which(is.na(overall_methyl))] <- 0
    bin.methyl[which(!is.na(overall_methyl))] <- 1
    ...
    strain.bin.mat <- apply(bin.methyl[,min.samples], 2, 
    ...
    bin.conf.mat[,min.samples] <- strain.bin.mat
    methyl_bin_cor[[state]] <- bin.conf.mat
}

saveRDS(methyl_expr_cor, methyl_cor_file)
saveRDS(methyl_bin_cor, methyl_bin_file)
```
...
### Presence/Absence of DNA methylation sites

The following plots show the association between DNA methylation
presence/absence gene expression using only DNA methylation in each
state.

```{r plot_bin_cor}
#pdf("~/Desktop/presence_absence.pdf")
if(is.interactive){quartz(width = 9, height = 9)}
par(mfrow = c(3,3), mar = c(2,4,2,2))
for(state in 1:num.states){
    #plot.new()
    #plot.window(xlim = c(-0.1, 1.1), ylim = c(-0.5, 0.6))
    plot.poly.xy(coords, methyl_bin_cor[[state]]["slope.lwr",],
    coords, methyl_bin_cor[[state]]["slope.upr",], col = state.col[state], new.plot = TRUE)
    points(coords, methyl_bin_cor[[state]]["slope.fit",], col = state.col[state], type = "l")    
    abline(h = 0)
    axis(2)
    mtext(side = 2, "Effect", line = 2.5)
    mtext(side = 3, paste("State", state), line = -1.5)
