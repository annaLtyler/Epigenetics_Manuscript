...
if(!file.exists(snp.table.file)){
    gene.snps <- lapply(1:length(common.transcripts), 
        function(x) get.sanger.snps(transcript.info.table[x,"chromosome_name"], 
        start.pos = transcript.info.table[x,"start_position"] - gene.buffer, 
        end.pos = transcript.info.table[x,"end_position"] + gene.buffer))
    names(gene.snps) <- common.transcripts
    saveRDS(gene.snps, snp.table.file)
}else{
