...
save_bed <- function(bed.table, filename){
  ...
  if(!file.exists(gz_file) || delete.previous){
...
if(!file.exists(strain.spec.enrich.file) || delete.previous){
...
pdf(here("Documents", "3.Manuscript", "Figures", "methylation_density.pdf"), 
  width = 7, height = 5)
  med_dist <- apply(avg_methyl_dist, 2, function(x) median(x, na.rm = TRUE))
  dist_se <- apply(avg_methyl_dist, 2, function(x) sd(x, na.rm = TRUE)/sqrt(length(which(!is.na(x)))))
  plot.new()
  plot.window(xlim = c(-0.5, 1.5), ylim = c(0, 550))
  plot.poly.xy(as.numeric(names(med_dist)), med_dist+dist_se, as.numeric(names(med_dist)), 
    med_dist-dist_se, col = "lightblue", border = NA)
  points(as.numeric(names(med_dist)), med_dist, type = "l", col = "gray40", lwd = 4)
abline(v = c(0, 1), lty = 2, col = "darkgray", lwd = 4)
axis(2, cex.axis = 2) 
dev.off()
```
...
```{r methyl_med_fig}
pdf(here("Documents", "3.Manuscript", "Figures", "methylation_percent.pdf"), 
  width = 7, height = 5)
percent_se <- apply(all.strain.mat, 2, function(x) sd(x, na.rm = TRUE)/sqrt(length(which(!is.na(x)))))
plot.new()
plot.window(xlim = c(-0.5, 1.5), ylim = c(0, 100))
methyl.med <- apply(all.strain.mat, 2, function(x) median(x, na.rm = TRUE))
plot.poly.xy(as.numeric(names(methyl.med)), methyl.med+percent_se, 
  as.numeric(names(methyl.med)), methyl.med-percent_se, col = "lightblue", border = NA)
points(as.numeric(names(methyl.med)), methyl.med, type = "l", col = "gray40", lwd = 4)
abline(v = c(0,1), lty = 2, col = "darkgray", lwd = 4)
axis(2, cex.axis = 2)
dev.off()
```
