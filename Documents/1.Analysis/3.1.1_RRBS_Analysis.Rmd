...
The following heat map shows all 
...
var_by_position <- sapply(1:ncol(methyl_mean), function(y) sapply(1:(length(mean.bins) - 1), function(x) mean(methyl_var[intersect(which(methyl_mean[,y] >= mean.bins[x]), which(methyl_mean[,y] <= mean.bins[(x+1)])),y], na.rm = TRUE)))
rownames(var_by_position) <- apply(cbind(mean.bins[1:10], mean.bins[2:11]),1, function(x) paste0(x[1], "-", x[2]))
colnames(var_by_position) <- round(as.numeric(colnames(methyl_mean)), 2)

boxplot(t(var_by_position), ylab = "Methylation SD", xlab = "Methylation Mean")
```
...
and the TSS has the lowest methylation on average.
...
plot.label = "Methylation Variation")
abline(v = c(0,1))
```
