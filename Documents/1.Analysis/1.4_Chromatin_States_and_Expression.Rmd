...
if(!file.exists(strain.expr.file) || delete.previous){
...
if(!file.exists(strain.scaled.expr.file) || delete.previous){
...
for(state in 1:num.states){
    ...
    state_peak_genes(binned.chromatin, state, min.n = 10, 
...
```{r plot_bin_enrich, results = "asis", fig.height = 15, fig.width = 8}
#pdf("~/Desktop/State.enrichment.pdf", width = 8, height = 15)
for(state in 1:num.states){
    cat("State", state, "\n")
    state.enrich <- state.bin.enrich[[state]]$enrichments
    names(state.enrich) <- round(as.numeric(names(state.enrich)), 2)
    which.enrich <- which(sapply(state.enrich, length) > 0)
    num.enrich <- length(which.enrich)
    if(num.enrich > 1){
        enrich.mat <- plot.enrichment.group(state.enrich, cluster_cols = FALSE, 
        transformation = "sqrt", max.term.size = 1000, 
        plot.label = paste("state", state), n.terms = 10)
        #wordcloud(unlist(strsplit(rownames(enrich.mat), " ")))
    }
    if(num.enrich == 1){
        plot.enrichment(state.enrich[[which.enrich]])
    }
    if(num.enrich == 0){
