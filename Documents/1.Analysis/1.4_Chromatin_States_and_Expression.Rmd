...
#pdf("~/Desktop/chromatin_states.pdf", width = 7, height = 4)
...
#pdf(here("Documents", "3.Manuscript", "Figures", "chromatin_correlation_by_state.pdf"), height = 8, width = 3)
...
To do this, we collected the position-centered chromatin matrices
for each strain

```{r center_mats}
within.strain.file <- here("Results", "ChromHMM", form.states, "Within_Strain_Expression_By_State_and_Position.RDS")

if(!file.exists(within.strain.file)){
    expr.order <- order.strains(colnames(cent.mats[[1]]), names(strain.mean.expr[[1]]), col.table)
    all.strain.by.position <- vector(mode = "list", length = 9)
    names(all.strain.by.position) <- colnames(cent.mats[[1]])
    #colnames(cent.mats[[1]])
    #names(strain.mean.expr[[1]])[expr.order]
    for(m in 1:9){
        strain.cent <- lapply(cent.mats[common.mat.locale], function(x) if(length(x) > 1){x[,m]}else{NA})
        strain.cent.mat <- plot.centered.vals(strain.cent, seq.by = 0.1, 
        ylim = c(1,8), return.means = FALSE)
        one.strain.expr <- sapply(rownames(strain.cent.mat), function(x) strain.mean.expr[[which(names(strain.mean.expr) == x)]][expr.order[m]])
        one.strain.scaled <- scale(one.strain.expr)
        all.state.pos.expr <- vector(mode = "list", length = num.states)
        names(all.state.pos.expr) <- 1:num.states
        for(s in 1:num.states){    
            state.pos.expr <- matrix(NA, nrow = nrow(strain.cent.mat), 
                ncol = ncol(strain.cent.mat))
                colnames(state.pos.expr) <- colnames(strain.cent.mat)
            for(p in 1:ncol(strain.cent.mat)){
                state.genes <- which(strain.cent.mat[,p] == s)
                gene.locale <- match(names(state.genes), rownames(strain.cent.mat))
                gene.expr <- one.strain.scaled[gene.locale]
                state.pos.expr[gene.locale,p] <- gene.expr
                }   
            #quartz()
            #boxplot(state.pos.expr, main = paste("State", s))
            #plot.centered.mat(state.pos.expr, ylim = c(-5, 5), plot.label = paste("State", s))
            #abline(v = c(0,1), h = 0)
            all.state.pos.expr[[s]] <- state.pos.expr
        }  
    all.strain.by.position[[m]] <- all.state.pos.expr
    }
saveRDS(all.strain.by.position, within.strain.file)
}else{
all.strain.by.position <- readRDS(within.strain.file)
}
```

The following plots show the mean expression of genes with the specified
state at the specified location. Each strain is shown separately, but they
all show the same relationship between chromatin state, position, and expression.
All expression was scaled within each strain, such that 0 is the mean expression.

These patterns are also very similar to the across-strain plots shown above.

```{r plot_within_strain, fig.height = 10, fig.width = 3.5, results = "asis"}
for(s in 1:length(all.strain.by.position)){
    cat("####", names(all.strain.by.position)[s], "\n")
    if(is.interactive){quartz(height = 12, width = 3)}
    par(mfrow = c(num.states, 1), mar = c(0,3,0,4))
    all.state.pos.expr <- all.strain.by.position[[s]]
    for(i in 1:num.states){
        one.state.col <- col2rgb(state.col[i])
        plot.new()
        plot.window(xlim = c(-1, 2), ylim = c(-1.5, 1.5))
        expr.mean <- colMeans(all.state.pos.expr[[i]], na.rm = TRUE)
        expr.num <- apply(all.state.pos.expr[[i]], 2, function(x) length(which(!is.na(x))))
        expr.se <- apply(all.state.pos.expr[[i]], 2, function(x) sd(x, na.rm = TRUE))/sqrt(expr.num)
        rel.pos <- as.numeric(colnames(strain.cent.mat))
        plot.poly.xy(poly.top.x = rel.pos, poly.bottom.x = rel.pos, 
        poly.top.y = (expr.mean + expr.se),
        poly.bottom.y = (expr.mean - expr.se), 
        col = rgb(one.state.col[1]/256, one.state.col[2]/256, one.state.col[3]/256, alpha = 0.5))
        abline(v = c(0,1), lwd = 2, lty = 2, col = "darkgray")
        abline(h = 0)
        axis(2)
        mtext(paste("State", i), side = 4, line = 2.5, srt = 270, cex = 1.5)
    }
    cat("\n\n")
}
```
