...
if(is.na(num.states)){
    num.states = 8; delete.previous = FALSE
    #num.states = 8; delete.previous = FALSE
}
```
...
if(delete.previous){
    delete.files <- list.files(here("Results", form.states, "ChromHMM", form.states), full.names = TRUE)
...
if(!file.exists(cis.coef.file)){
    all.cis.coef <- t(sapply(1:length(do.eqtl), 
    function(x) get.cis.coef(do.eqtl, do.eqtl.info, x)))
    colnames(all.cis.coef) <- c("transcript.id", "nearest.marker", "LOD")
...
## Enrichment Group Examples {.tabset .tabset-fade .tabset-pills}

We looked at examples of groups of genes annotated to specific terms
to see if they had similar chromatin patterns. I haven't had much luck
with this line of inquiry, but I'm keeping this code here just in case.

```{r gene_group_examples, results = "asis", fig.width = 7, fig.height = 8}
#ex.genes <- enrichment.genes(state.enrich, "extracellular matrix")
#ex.genes <- enrichment.genes(state.enrich, "ion binding")
ex.genes <- enrichment.genes(state.enrich, "kidney development")
ex.gene.names <- lapply(ex.genes, 
    function(x) rnaseq.gene.info[match(x, rnaseq.gene.info[,"ensembl_gene_id"]), "external_gene_name"])

for(i in 1:length(ex.gene.names)){
    ex.gene.id <- ex.gene.names[[i]]
    state <- as.numeric(names(ex.gene.names)[i])
    for(g in 1:length(ex.gene.id)){
        cat("###", ex.gene.id[g], "\n")
        plot.one.gene(do.eqtl, rnaseq.gene.info, rna.seq, all.bed, col.table, 
        gene.name = ex.gene.id[g], T.or.C, start.feature, end.feature, upstream.buffer, 
        downstream.buffer, separate.windows = is.interactive, dim.rd = state.prop, state = state, 
...
```{r examples, fig.width = 7, fig.height = 8}
...
```{r state8, eval = FALSE}
#keep this section for playing around with examples
example.genes <- find_example_cor(state.expr.window.p, sig.val, 8, 1.1)
example.names <- rnaseq.gene.info[match(example.genes, rnaseq.gene.info[,"ensembl_gene_id"]),"external_gene_name"]
enrich <- gost(example.names, organism = "mmusculus", evcodes = TRUE)
plot.enrichment(enrich)

ex.genes <- enrichment.genes(enrich, "Ribosomal", FALSE)$result
for(g in 1:length(ex.genes)){
    cat("###", ex.genes[g], "\n")
    plot.one.gene(do.eqtl, rnaseq.gene.info, rna.seq, all.bed, col.table, 
    gene.name = ex.genes[g], T.or.C, start.feature, end.feature, upstream.buffer, 
    downstream.buffer, separate.windows = is.interactive, dim.rd = state.prop, state = 8, 
    total.states = num.states, show.state.numbers = FALSE)
    cat("\n")
}
```
