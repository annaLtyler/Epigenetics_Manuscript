...
max.chrom <- apply(chrom.r2, 1, function(x) if(!all(is.na(x))){max(x, na.rm = TRUE)}else{NA})
...
r2.list <- list("Haplotype" = max.eqtl, "Chromatin_State" = max.chrom, "SNP" = max.snp,
...
plot(density(common.list[[1]]), xlab = "Variance Explained", 
    main = "", type = "n", ylim = c(0, 11.5))
...
common.transcripts <- Reduce("intersect", list(rownames(chrom.r2), rownames(eqtl.r2), 
    names(methyl.r2), names(max.snp)))
common.chrom.idx <- match(common.transcripts, rownames(chrom.r2))
...
get_chrom_scan <- function(gene.id){
    chrom.idx <- which(rownames(chrom.r2) == gene.id) 
...
plot_together <- function(gene.id){
    ...
    global.max.r2 <- max(c(gene.chrom.r2, gene.snp.r2, gene.methyl.r2, 
        hap.r2), na.rm = TRUE)
    ...
    segments(x0 = global.min.pos, x1 = global.max.pos, y0 = gene.chrom.r2,
        col = CCcolors)
    #points(x = as.numeric(rownames(gene.chrom.r2)), gene.chrom.r2, pch = "+", 
    #    col = "#377eb8", cex = 2)
...
for(i in 1:3){
    ...
    sample.genes <- sample(names(max.r2), n.sample)
