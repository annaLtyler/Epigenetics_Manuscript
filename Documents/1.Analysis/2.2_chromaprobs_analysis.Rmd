...
## Diet-interacting eQTLs
Here we investigate which eQTLs are influenced by diet. 
In the previous workflow, we calculated LOD scores with and without diet 
as an interacting covariate. Here we show those LOD scores plotted against 
each other.

```{r diet_int}
par(mfrow = c(1,2))
plot(e.lod[,1:2], main = "Genetic eQTL LOD scores")
abline(0,1)

max.ch.lod <- t(sapply(ch.lod, function(x) if(length(x) > 0){apply(x, 2, function(y) max(y, na.rm = TRUE))}else{rep(NA, 4)}))
plot(max.ch.lod[,1:2], main = "Chromatin State eQTL LOD Scores")
abline(0,1)

plot(e.lod[,4], max.ch.lod[,4])

pheatmap(cor(e.lod, use = "pairwise.complete.obs"))
```
