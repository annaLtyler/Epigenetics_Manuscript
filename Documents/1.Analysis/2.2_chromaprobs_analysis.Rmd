...
RNA and non-coding RNA processing. 
...
centered.lod.file <- here("Results", "chQTL", "centered.LOD.RDS")
...
```{r state_effect_fig, eval = FALSE}

pdf(here("Documents", "3.Manuscript", "Figures", "DO_chromatin_effects.pdf"), width = 3, height = 10)
xmin = -0.2;xmax = 1.2;ymin = -0.25; ymax = 0.25
par(mfrow = c(num.states, 1), mar = c(0.5,3,0.5,2))
#plot the across-strain effects
for(state in 1:num.states){
    state.effects <- all.state.coef[[state]]
    coords <- as.numeric(colnames(state.effects))

    mean.effects <- apply(state.effects, 2, function(x) mean(x, na.rm = TRUE))
    effect.sd <- apply(state.effects, 2, function(x) sd(x, na.rm = TRUE)/sqrt(length(which(!is.na(x)))))
    
    plot.new()
    plot.window(xlim = c(xmin, xmax), ylim = c(ymin, ymax))
    plot.poly.xy(coords, mean.effects+effect.sd, coords, mean.effects-effect.sd, 
    col = state.col[state])
