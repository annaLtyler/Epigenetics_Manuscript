...
## LOD score differences

The following plot shows the distribution of the difference between the
genetic eQTL and the chromatin eQTL. There are a few genes for 
which the chromatin eQTL has a higher LOD score than the genetic eQTL.
All of these genes have very low genetic LOD scores, and the chromatin
LOD scores are not very high either.

```{r lod_diff}
lod.diff <- e.lod-max.ch.lod
hist(lod.diff, breaks = 100)
better.chrom.locale <- which(e.lod-max.ch.lod < 0)
better.chrom.lod.names <- gene.names[better.chrom.locale]
better.chrom.lod <- e.lod[better.chrom.locale]
better.chrom.ch.lod <- max.ch.lod[better.chrom.locale]
```
...
for(s in 1:8){
  ...
  all.state.mag[[s]] <- plot.centered.vals(non.missing.state, seq.by = 0.01, min.representation = 10, 
  ylim = c(0, 0.5), plot.label = paste("State", s), 
...
for(s in 1:8){
  ...
  all.state.var[[s]] <- plot.centered.vals(non.missing.state, seq.by = 0.01, min.representation = 10, 
  ylim = c(0, 1.1), plot.label = paste("State", s), 
