...
other very well. However, there is also a fairly large group of genes for which
there is a mismatch between the two associations. The distribution of those 
differences is shown in the right-hand panel.
...
par(mfrow = c(1,2))
plot(e.lod[,1], max.ch.lod[,1], pch = 16, col = "gray", cex = 0.8,
...
We have multiple hypotheses that could explain these mismatches. We will go 
through each to see if we can describe this group of genes. For each
hypothesis, we are not necessarily interseted in describing the full
set of genes, but rather determining whether there are any genes in the
mismatch group that fit the criteria. The hypotheses are as follows:

1. Altered chromatin-based regulation due to high-fat diet:
A high fat diet may alter the chromatin regulation of some genes in the
liver. We would expect these genes to have a diet-interactive chromatin-based
eQTL. If these changes can be explained by a diet-interactive genetics-based
eQTL, does that 
...
To test whether there were transcripts whose regulation by chromatin
is affected by high-fat diet, we modeled each transcript with diet as
an additive covariate and as an interactive covariate, and examined
the difference in LOD score between the two models. 

The figures below show the LOD scores from the two models plotted 
against each other both for genetic-based eQTLs (left), and 
chromatin-based eQTLs (right).

Almost all eQTLs had higher LOD scores when diet was used as an 
interactive covariate. However, some transcripts were affected
more than others. 
