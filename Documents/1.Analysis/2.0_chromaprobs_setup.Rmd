...
if(!data.loaded){
  do.data <- load(here("Data", "DOQTL", "Svenson_DO850_for_eQTL_veiwer_v0.Rdata"))
}
expr.data <- list(dataset.expr.petr$expr, dataset.expr.petr$covar)
names(expr.data) <- c("expr", "covar")
...
expr.ind.locale <- match(common.ind, rownames(expr))
...
```{r expand_chrom}
chrom.mats <- readRDS(here("Results", "ChromHMM", "008_states_C", 
"Chromatin_States_8_full_gene_1000.RData"))

expand_chrom <- function(chrom.mat, num.states = 8){
...
exp.chrom <- lapply_pb(chrom.mats, expand_chrom)
