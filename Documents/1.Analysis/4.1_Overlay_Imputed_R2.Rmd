...
max.methyl <- sapply(methyl.r2, function(x) if(length(x) > 1){max(x, na.rm = TRUE)}else{NA})
...
get_snp_alleles <- function(gene.id){
    ...
    trimmed.alleles <- allele.calls[,has.diff]
...
plot_together <- function(gene.id){
    ...
    layout(layout.mat, heights = c(1,0.5, 1, 1), widths = c(1,0.4))
...
same.r2 <- intersect(which(apply(all.r2, 1, function(x) x[3] == x[4])), which(all.r2[,3] > 50))
gene.id <- sample(rownames(all.r2)[same.r2], 1)
plot_together(gene.id)

snp.over.chrom <- intersect(which(apply(all.r2, 1, function(x) x[3] > x[2])), which(all.r2[,1] > 20))
length(snp.over.chrom)
gene.id <- sample(rownames(all.r2)[snp.over.chrom], 1)
plot_together(gene.id)

chrom.over.snp <- intersect(which(apply(all.r2, 1, function(x) x[3] < x[2])), which(all.r2[,1] > 20))
length(chrom.over.snp)
gene.id <- sample(rownames(all.r2)[chrom.over.snp], 1)
plot_together(gene.id)
```
