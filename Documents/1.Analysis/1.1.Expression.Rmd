...
if(!rna.seq.loaded){
    ...
    #filter to transcript that have at least 1 reads in at least 50% of individuals
    min.reads = 1
    min.prop = 0.5
    pass.filter <- apply(rna.seq, 1, function(x) length(which(x >= min.reads))/length(x) > min.prop)
    rna.seq <- rna.seq[which(pass.filter),]
}
