...
We looked for states that were significantly related to expression together
in a single gene.

```{r state_comb}
sig_state_comb <- apply(all.prop.p, 1, function(x) which(x <= sig.val))
not.null <- which(sapply(sig_state_comb, length) > 0)
```

There were `r length(not.null)` genes for which at least one state was correlated
with expression at a $p$ value $\leq$ `r sig.val`.

The largest proportion of these had expression significantly correlated with only 
one chromatin state. After that, pairs of states were correlated. Mostly, 1 and 7,
5 and 7, and 7 and 8.


```{r occur_mat}
co.occur <- sapply(sig_state_comb[not.null], function(x) paste(x, collapse = "_"))
co.occur.counts <- table(unlist(co.occur))
over.one <- which(co.occur.counts > 5)
#quartz(height = 10, width = 5)
barplot(sort(co.occur.counts[over.one]), las = 2, horiz = TRUE)

all.sig <- all.prop.r
all.sig[which(all.prop.p > 0.05)] <- NA
boxplot(all.sig)
```
