...
This is the second script in the series that analyzes epigenetic data from the Genetics of 
Epigenetics Project at JAX. The first scripts (1.ChromHMM.Rmd) started with bam files and
processed them through ChromHMM. This workflow also uses results from the Expression.Rmd
workflow.
...
num.states <- 8
sig.val = 0.05
offline = FALSE #set to TRUE to avoid all functions that use the internet
                #some files may need to be generated ahead of time to use 
                #this setting
is.interactive = FALSE #keep this as FALSE when running render()
                    #use TRUE if running interactively. It plots
                    #one gene data in multiple windows
selected.mark <- NULL #set to null if no specified marks
...
data.dir <- here("results", form.states)
analysis.dir <- here("results", "analysis")
if(!file.exists(analysis.dir)){dir.create(analysis.dir)}
results.dir <- here("results", "analysis", form.states)
...
#file containing information about expressed transcripts, gene name, genomic position, etc.
#This will not change from run to run, so we'll put the results in data
gene.info.file <- here("data", "support_files", "RNASeq_gene_info.RData")

#file containing strain averages for each transcript. These
#do not change from run to run. Save in data
strain.expr.file <- here("data", "support_files", 
paste0("Strain_Mean_", T.or.C, "_Expression.RData"))

#file containing r and p values indicating correlation between state proportions and expression values
states.expression.file <- here("results", "analysis", form.states,
paste0("State_", num.states, "_Prop_Expression_Cor_", gene.text, "_", buffer.text, ".RData"))

#file containing r and p values indicating correlation between mds states and expression values
state.mds.expression.file <- here("results", "analysis", form.states,
paste0("State_", num.states, "_MDS_Expression_Cor_", gene.text, "_", buffer.text, ".RData"))

#The cis coefficients for all transcripts in DO population
cis.coef.file <- here("data", "support_files", "cis.coef.RData")

state.enrichment.file <- here("results", "analysis", form.states, "enrichment.by.state.RData")
```

```{r clear_results, eval = FALSE}
#If evaluated, this chunk will clear out all results generated
#by this script to trigger recalculation of all results
#delete.files <- list.files(here("results", form.states, "analysis", form.states), 
#full.names = TRUE)
...
comparing different names as well as for defining strain colors. Load this table:
...
all.var <- ls()
#most current library
#stable archived library
...
rna.seq.file <- here("data", "support_files", "9StrainsEffCts_vst.RDS")
...
do.qtl.file <- here("data", "support_files", "DOQTL", "DO478_tpm_20360_cis_allele_coef.RData")
...
## ChromHMM
We are analyzing the chromatin states based on states output by ChromHMM. 
...
chrom.state.file <- here("results", "analysis", form.states, 
...
chrom.prop.file <- here("results", "analysis", form.states,
...
chrom.mds.file <- here("results", "analysis", form.states,
...
The following plot shows the distribution of state proportions across genes. Each 
row is a single gene. Each column is one state for one strain.

Most of the genes have high proportions of state 1 (no marks).
Other states that dominate blocks of genes are states 2 and 7. 
There is a small block dominated by state 3, and another small block of state 8.
State 8 also shares blocks of genes with state 7.
...
state.prop.heat.file <- here("results", "analysis", form.states,
...
emissions.file <- here("results", form.states, paste0("emissions_", num.states, ".png"))
cat("![](",emissions.file,")")
```
