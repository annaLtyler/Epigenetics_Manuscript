...
was DBA/2J, which, along with C57BL/6J, is one of the founders 
...
demonstrate that strain variation in chromatin state, paired 
with gene expression is useful for annotation of functional 
regions of the mouse genome. Finally, we provide, to our 
knowledge, the first data resource that documents variation in 
...
[@lawrence2016lateral; @jones2012functions; @moore2013dna]. Across 
cell types, unique combinatorial patterns of histone modifications 
mark chromatin states that establish cell type-specific patterns of 
gene expression [@pmid20657582, @pmid21441907]. Similarly, the methylation 
of CpG sites around gene promoters and enhancers influences 
transcription in a cell type-specific manner [@pmid21701563; @pmid20720541]. 

Patterns of histone modifications and DNA methylation are established 
during development. The result is a canonical epigenetic landscape 
for coordination of major patterns of gene expression 
for each cell type (for review, see [@pmid32671792; @pmid29625185]). 
As an organism ages and responds to its environment, patterns of both 
histone modifications 
[@pmid33153221; @pmid28820059; @pmid24459735; @pmid17559965] and of 
DNA methylation [@pmid30419258; @pmid25637097] change. Variation in 
epigenetic modifications have been linked to premature aging 
[@pmid27259148; @pmid16738054], autism [@pmid19896504; @pmid19264732], 
cancer [@pmid20574448; @pmid19892027], and neurological diseases among 
others [@pmid22473383].

Epigenetic modifications coordinate allow a single genome to 
give rise to many different types of cells with diverse morphology 
and physiology. This amazing feature of epigenetic modifications 
has been intensely studied, and the variation in epigenetic 
landscapes across cell types has been extensively documented 
[@pmid21441907; @pmid25693563]. Less well understood, however, is 
the role that genetic variation plays in determining epigenetic 
landscapes. 
...
gene expression varies within the global constraints of cell type. 
This variation can increase or decrease an organism's risk of 
...
Expression (GTEx) Project [@pmid32913075 @pmid32913073] have identified 
and catalogued many such loci in humans, and countless independent 
...
modifications will increase our understanding of the mechanisms of 
gene regulation, provide insights into the mechanisms establishing 
cell type-specific epigenetic landscapes, and to improve the functional 
annotation of the genome as it relates to the regulation of gene 
expression. The vast majority of SNPs associated with human
disease traits are located in non-coding regions, suggesting that they
influence gene regulation, rather than protein function 
[@pmid21617055; @pmid19474294]. However, annotation of these regions 
is difficult without additional genomic features, such as histone 
modifictions and DNA methylation. Overlaying a map of variation in 
epigenetic features has the potential to provide a picture of how 
genetic variation changes functional elements, like enhancers and 
insulators, in the genome [@pmid23595227; @pmid20657582].
...
with relatively few cells [@pmid20077036], thus opening the door to 
the possibility of cataloging epigenetic variation across more cell 
types and individuals. Here, we performed a survey of epigenetic 
variation in hepatocytes across nine inbred mouse strains. We 
included the eight founders of the Diversity Outbred/Collaborative 
Cross (DO/CC) [@Svenson:2012hq] mice, as well as DBA/2J, which, along 
with C57BL/6J, is one of the founders of the widely used BxD recombinant 
inbred panel of mice [@Ashbrook:2019bd]. We assayed four histone 
modifications (H3K4me1, H3K4me3, H3K27me3, and H3K27ac), as well as 
DNA methylation. We used ChromHMM [@Ernst:2012ii] to identify 
`r num.states` chromatin states, classified by unique combinations of 
the four histone marks, and investigated the association between 
variation in these states and variation in gene expression across the 
nine strains. We separately investigated the relationship between 
...
Care and Use Committee (The Jackson Laboratory, Protocol XXX).
...
Cross/Diversity Outbred mice [@Chesler:2008ge]. The ninth strain, DBA/2J, 
...
protease inhibitor cocktail (Roche) to prevent histone protein degradation. 
...
We confirmed digestion of nucleosomes into 150bp fragments with with 
...
Dynabead Protein G beads and histone antibodies. After binding to antibodies, 
samples were washed to remove unboud chromatin and then eluted with high-salt 
buffer and proteinase K to digest protein away from DNA-protein complexes.
The DNA was purified using the Qiagen PCR purification kit. Quantification 
was performed using the Qubit quantification system (See Supplemental Methods).

## Diversity Outbred mice
We used previously published data from a population of 478 diversity outbred (DO)
mice [@Svenson:2012hq] to compare to the data collected from the inbred 
...
### Genotyping
All DO mice were genotyped as described in @Svenson:2012hq using
...
as described in @Gatti:2014ko. The MUGA and MegaMUGA arrays
were merged to create a final set of evenly spaced 64,000 interpolated 
markers. 

### Tissue collection and gene expression
At sacrifice, whole livers were collected and gene expression was
measured using RNA-Seq as described in [@pmid27309819; @pmid28592500]. 
Transcript sequences were aligned to strain-specific genomes, and we 
used an expectation maximization algorithm (EMASE) to estimate read 
counts (\url{https://github.com/churchill-lab/emase}).
...
mapped to strain-specific pseudogenomes, which integrate known SNPs 
and indels from each strain. The B6 samples were aligned directly 
to the reference mouse genome. The pseudogenomes were created using 
[EMASE](https://github.com/churchill-lab/emase). We used Bowtie 
[@pmid21154709] to align and map reads from the RNA-Seq and ChIP-Seq 
experiments. 

### Transcript quantification
We quantified gene expression using edgeR [@pmid19910308],
and transcripts with less than 1 CPM in two or more replicates 
were filtered out. Transcripts were further filtered to include 
only protein-coding transcripts.

We used the R package sva [@sva] to perform a variance
stabilizing transformation (vst) on the RNA-Seq read 
counts from both inbred and outbred mice. In the inbred 
mice we used a blind transformation, while in the outbred 
mice, we included DO wave and sex in the model. For eQTL 
mapping, we performed rank Z normalization on the RNA-Seq 
read counts across transcripts from the outbred mice.

### ChIP-Seq quantification
We used MACS 1.4.2 [@pmid18798982] to identify peaks in the ChIP-Seq sequencing
data, with a significance threshold of $p \leq 10^{-5}$. In order to compare 
peaks across strains, we converted the MACS output peak coordinates to common 
B6 coordinates using g2g tools (https://churchill-lab.github.io/g2gtools/). 

## Quantifying DNA methylation
RRBS data were processed using a bismark-based pipeline modified from 
[@pmid30348905]. The pipeline uses Trim Galore! 0.6.3 
...
bowtie2 as implemented in Bismark 0.22 [@pmid21493656]. The pseudogenomes were
...
were aligned directly to the reference mouse genome.


## Analysis of histone modifications

### Identification of chromatin states
We used ChromHMM [@pmid29120462] to identify chromatin states, which are
...
were provided by RefSeq [@pmid26553804] and included with the release of ChromHMM, 
which we downloaded on December 9, 2019 [@pmid29120462].
...
from OregAnno [@pmid26578589] using the UCSC genome browser [@pmid12045153] on May 4, 2021.
* **Promoters** - We downloaded promoter coordinates provided by the eukaryotic
promoter database [@pmid27899657; @pmid27899657], through the UCSC genome browser on April
26, 2021.
...
downloaded cCRE annotations provided by ENCODE [@pmid22955616; @pmid32728249] 
...
2kb from their nearest neighbor, for a final set of 14,048 genes.
...
analagous to the genoprobs object in R/qtl2 [@pmid30591514]. The genomic 
...
[@pmid1921910; @pmid21921916], on July 6, 2021.
...
Gene expression varied reproducibly across inbred strains of 
...
The states were enriched around known functional elements in 
the mouse genome (Figure \ref{fig:state_overview}B). 
...
state on gene expression in hepatocytes. These effects tended to be of 
the same sign and greater magnitude than the across-strain effects.

The states in Figure \ref{fig:state_overview} are shown in order of their
effect on expression, which helps illustrate several patterns. 
...
states `r active.promoter.state` and `r active.enhancer.state` were 
...
DNA methylation showed similarly dramatic variation in abundance
...
were similarly related to variation in expresion across strains.
...
TSS. These results imply that although percent DNA methylation 
is used in gene regulation within a cell type, it is not heritable 
...
and SNPs into DO mice (Methods). Chromatin state is largely determined by 
local genotype, especially early in life [@pmid16009939], and can thus be 
reliably imputed from local genotype. Further, we have shown here that 
local chromatin state correlates with variation in gene expression across 
inbred strains. DNA methylation, on the other hand, is known not to be highly 
heritable [@pmid33931130], and thus cannot be reliably imputed from local 
...
\ref{fig:example_gene}A that are aligned with the two red regions, 
which we suggest are putative enhancer regions. 
...
altering chromatin state. But SNPs downstream of this region underly invariant 
chromatin. The downstream enhancer has no underlying SNPs, suggesting that 
there is an alternative mechanism for determining chromatin state at this 
location.
...
all transcripts are shown in Figure \ref{fig:effect_distrubutions}. These 
...
the DO population ($R^2 = `r mean.var.exp["Methylation_R2"]`$), and had a 
...
in inbred mice mirrors genetic variation. We further showed
...
which can then be linked to variation in gene expression. 
The \textit{Pkd2} example illustrates how genetic and epigenetic
variation can be combined to identify two putative enhancer
regions for the gene. The variation in chromatin state further
offers a putative mechanism for the observation of a cis-eQTL at 
the level of the haplotype.
 
The discordance between the patterns of chromatin state and 
SNPs in this gene are particularly interesting. The variation
in chromatin state is present in the absence of local SNPs, 
whereas further downstream, variation in SNPs does not correspond
to variation in chromatin state. This suggests that the presence
of the downstream enhancer is determined by another mechanism, 
perhaps SNPs acting in \textit{trans} to this region, or local 
variation that was not measured--structural variation, for example.
...
is invariant -- the CpG island at the TSS is unmethylated in all 
strains. Thus, although chromatin state appears to be highly 
influenced by local genotype, percent DNA methylation is not.

Similar observations have been made in human studies [@pmid33931130].
Multiple twin studies have estimated the average heritability of 
individual CpG sites to be roughly 0.19 
[@pmid27051996; @pmid24183450; @pmid22532803], with only about 
10\% of CpG sites having a heritability greater than 0.5 
[@pmid24183450; @pmid22532803; @pmid24887635]. Trimodal CpG sites, 
...
were associated with local eQTL [@pmid20485568]. Here, we did not 
...
completely distinct spatial distribution. State `r active.promoter.state` 
was distributed in a very narrow band centered on the TSS, while 
state `r active.enhancer.state` was distributed across a much broader r
egion centered upstream of the TSS. State `r strong.intragenic.enhancer` 
had a completely different distribution -- it was depleted at the TSS, 
and most abundant within the gene body and near the TES. This variation 
in spatial distribution was mirrored in the spatial effects on transcription. 
State `r active.promoter.state`, which we annotated as an active promoter, 
was positively associated with transcription when it was present at the TSS. 
In contrast, states `r strong.intragenic.enhancer` and `r active.enhancer.state`, 
which we annotated as enhancers, were associated with increased transcription
when present anywhere in the gene body. We would not be able to detect such 
patterns if analyzing the histone modifications in isolation. These results 
highlight the complexity of the histone code and the importance at analyzing 
...
[@pmid15680324; @pmid14661024; @pmid12353038; @pmid16728976], so the 
...
by a combination of activating and repressive histone modifications,
and are usually associated with undifferentiated cells 
[@pmid23788621; @pmid22513113]. Here we identified two bivalent 
...
[@pmid23788621, @pmid22513113]. These promoters have also been shown 
...
[@pmid27800026]. Poised enhancers are also observed during differentiation 
and in differentiated cells [@pmid32432110]. In concordance with these 
...
one of the first rigorous resources that explores the connection 
between genetic variation and epigenetic variation.
