...
explained. Example results for the gene \textit{Pkd2} are shown in
Figure XXX. Panels B, C, and D show the chromatin state, SNP genotype,
and DNA methylation status at multiple positions along the gene body in
the inbred strains. The boundaries of the gene body along with the
direction of transcription are shown by the arrow under panel A. Panel A
shows the variance explained by each genomic feature at each position
with a distinct symbol. The variance explained by the local haplotype is
shown as the dashed line at the top of the panel. There are two
particularly interesting regions in this gene. One is at the TSS and the
immediately surrounding area, and the other is just downstream of the
TSS.

Both regions show variation in chromatin state that is highly correlated
with gene expression in the DO (blue plus signs in top panel). Panel B,
which is in the same order as the haplotype effect shown in panel E,
shows that chromatin states that are associated with higher expression
(red states) are more abundant in the haplotypes that are associated
with higher expression in the DO. The two strains with the least red in
the chromatin state matrix, WSB and PWK, are the lowest-expressing
alleles in the DO. The two strains with the most red, CAST and NOD, are
the strains with the highest expressing alleles in the DO. This basic
pattern separating the two lowest expressing strains from the remaining
strains with high expression is mirrored in the gene expression in the
inbred mice (Figure XXXF)
...
these critical regions, and does not contribute to variation in
expression across genetically distinct individuals.
