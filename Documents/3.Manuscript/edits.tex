...
%DIF DEL Epigenetics_Manuscript_resubmission1.tex   Sat Feb 25 10:19:42 2023
%DIF ADD Epigenetics_Manuscript.tex                 Fri Apr  7 14:33:51 2023
...
%DIF < \pagestyle{fancy}
%DIF < \lhead{Tyler, Spruce, et al.}
%DIF < \rhead{Epigenetic variation in nine inbred mouse strains}
%DIF -------
...
\citep{kurtz2020diversity}. These models have \DIFdelbegin \DIFdel{huge }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend potential to uncover
...
documented, the epigenetic diversity of these \DIFdelbegin \DIFdel{mice }\DIFdelend \DIFaddbegin \DIFadd{strains }\DIFaddend is relatively
unknown.
\DIFdelbegin \DIFdel{This gap in knowledge is potentially critical to understanding
...
modifications \DIFdelbegin \DIFdel{allow an organism
to develop an array of diverse cell types }\DIFdelend \DIFaddbegin \DIFadd{vary across cell types allowing organisms to develop all
of their diverse cells }\DIFaddend from a single genome. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Epigenetic modifications
also }\DIFdelend \DIFaddbegin \DIFadd{Epigenetic modifications
have also been shown to }\DIFaddend vary across individuals \DIFdelbegin \DIFdel{. Studies }\DIFdelend in humans
\citep{mcvicker2013identification, kang2021genetic}, rats
\citep{rintisch2014natural}, \DIFdelbegin \DIFdel{and cattle \mbox{%DIFAUXCMD
...
\mbox{%DIFAUXCMD
\citep{kang2021genetic, rintisch2014natural, prowse2022genetic} }\hspace{0pt}%DIFAUXCMD
and
clinical outcomes \mbox{%DIFAUXCMD
\citep{kang2021genetic}}\hspace{0pt}%DIFAUXCMD
. Similar studies have
associated heritable variation in DNA methylation to clinical outcomes, such as immune dysregulation \mbox{%DIFAUXCMD
\citep{hawe2022genetic}}\hspace{0pt}%DIFAUXCMD
.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{This link between genetic variation and epigenetic variation may be a
critical piece of understanding genetic susceptibility to disease.
}\DIFdelend \DIFaddbegin \DIFadd{cattle \mbox{%DIFAUXCMD
\citep{prowse2022genetic}}\hspace{0pt}%DIFAUXCMD
, and mice
\mbox{%DIFAUXCMD
\citep{link2018analysis, schilling2009allele, zhou2022dna, grimm2019dna, xie2012base, gujar2018profiling}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend Because the majority of disease-associated genetic variants discovered
...
\DIFdelbegin \DIFdel{Detailed epigenomic landscapes, therefore, may provide important
mechanistic insight connecting genotype to
disease risk}\DIFdelend \DIFaddbegin 

\DIFadd{Supporting this hypothesis, it has been shown that epigenetic variation
across individuals is associated with variation in gene expression
\mbox{%DIFAUXCMD
\citep{kang2021genetic, rintisch2014natural, prowse2022genetic} }\hspace{0pt}%DIFAUXCMD
and
clinical outcomes \mbox{%DIFAUXCMD
\citep{kang2021genetic, hawe2022genetic}}\hspace{0pt}%DIFAUXCMD
. Work in mice
has demonstrated variability of both DNA methylation and histone
modifications across inbred strains, including some of the DO/CC
founders
\mbox{%DIFAUXCMD
\citep{link2018analysis, schilling2009allele, zhou2022dna, grimm2019dna, xie2012base, gujar2018profiling}}\hspace{0pt}%DIFAUXCMD
.
This work has shown that strain-specific epigenetic variation is
heritable \mbox{%DIFAUXCMD
\citep{schilling2009allele, grimm2019dna} }\hspace{0pt}%DIFAUXCMD
and leads to
differential gene regulation and differential cellular phenotypes across
strains \mbox{%DIFAUXCMD
\citep{link2018analysis}}\hspace{0pt}%DIFAUXCMD
. Therefore, a broader survey of the
DO/CC founders is potentially critical to understanding mechanisms of
gene regulation and its impact on disease in these important models}\DIFaddend .

To \DIFdelbegin \DIFdel{address the lack of }\DIFdelend \DIFaddbegin \DIFadd{extend }\DIFaddend documented epigenetic variation \DIFdelbegin \DIFdel{in the }\DIFdelend \DIFaddbegin \DIFadd{to all }\DIFaddend DO/CC founders, we
...
We extended our analysis into a population of \DIFdelbegin \DIFdel{DO }\DIFdelend \DIFaddbegin \DIFadd{Diversity Outbred (DO)
\mbox{%DIFAUXCMD
\citep{churchill2012diversity, Svenson:2012hq, Gatti:2014ko, pmid27309819}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend mice to investigate the heritability of histone modifications and DNA
...
which \DIFdelbegin \DIFdel{expression
quantitative trait loci (eQTLs) }\DIFdelend \DIFaddbegin \DIFadd{gene expression }\DIFaddend in the DO \DIFdelbegin \DIFdel{were related to variation in
epigenetic modifications}\DIFdelend \DIFaddbegin \DIFadd{mice corresponded with imputed
epigenetic variation}\DIFaddend .
...
\ref{fig:state_overview}B\DIFaddbegin \DIFadd{, Supp. Fig. }\DIFaddend ), and correlated the presence of
each state with gene expression both across genes and across the inbred
...
functional element. \DIFaddbeginFL \DIFaddFL{Rows were scaled to run between 0 and 1
for ease of visualization. }\DIFaddendFL Abbreviations are as follows: 
...
\begin{figure}[ht!]
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.67\textwidth]{Figures/Fig4_Abundance.png} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{Figures/Fig4_Abundance.png} 
...
independently. \DIFdelbegin \DIFdel{This is not the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend standard method for testing associations \DIFdelbegin \DIFdel{between markers and gene expression in this type of population, but it
constrains }\DIFdelend \DIFaddbegin \DIFadd{is to
include all alleles (or chromatin states) in a single linear model.
However, because there are varying numbers of alleles across modalities
(eight haplotypes, 14 chromatin states, three DNA methylation values,
and two SNPs), we needed to constrain }\DIFaddend the degrees of freedom for all
features\DIFdelbegin \DIFdel{to one so we can
compare the associations across all features.
}\DIFdelend \DIFaddbegin \DIFadd{. Thus, for all features, we tested only a single haplotype,
chromatin state, etc. in each model.
}\DIFaddend 
...
\caption{\DIFdelbeginFL \DIFdelFL{Gene expression }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Comparison of the }\DIFaddendFL variance \DIFaddbeginFL \DIFaddFL{explained }\DIFaddendFL in \DIFdelbeginFL \DIFdelFL{a }\DIFdelendFL DO \DIFdelbeginFL \DIFdelFL{population explained 
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{gene expression
}\DIFaddendFL by \DIFdelbeginFL \DIFdelFL{chromatin state compared to three other }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{four }\DIFaddendFL genomic features: \DIFdelbeginFL \DIFdelFL{local }\DIFdelendFL haplotype \DIFaddbeginFL \DIFaddFL{(Hap.) chromatin state (Chrom.)}\DIFaddendFL , 
local SNP genotype \DIFaddbeginFL \DIFaddFL{(SNP)}\DIFaddendFL , and local imputed DNA methylation status 
\DIFaddbeginFL \DIFaddFL{(DM)}\DIFaddendFL . A. Distributions of gene expression variance explained by each 
...
and DNA methylation. \DIFdelbegin \DIFdel{Interestingly, even }\DIFdelend \DIFaddbegin \DIFadd{Even }\DIFaddend though DNA methylation showed no association
...
\DIFdelbegin %DIFDELCMD < \hypertarget{hypothesis-generation-for-cis-regulatory-regions}{%
%DIFDELCMD < \subsection{Hypothesis generation for cis-regulatory
%DIFDELCMD < regions}\label{hypothesis-generation-for-cis-regulatory-regions}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{hypothesis-generation-for--regulatory-regions}{%
...
\DIFdelbegin \DIFdel{cis-regulatory }\DIFdelend \DIFaddbegin \textit{\DIFadd{cis}}\DIFadd{-regulatory }\DIFaddend regions in these mice. In particular, for any
...
a \DIFdelbegin \DIFdel{cis-regulatory }\DIFdelend \DIFaddbegin \textit{\DIFadd{cis}}\DIFadd{-regulatory }\DIFaddend element. This occurrence provides an
...
\DIFdelbegin \DIFdel{identifies two particularly interesting }\DIFdelend \DIFaddbegin \DIFadd{identified two }\DIFaddend regions marked by gray arrows in panel A. One is at the
...
chromatin state maps offers \DIFdelbegin \DIFdel{an interesting way to identify
cis-regulatory }\DIFdelend \DIFaddbegin \DIFadd{a potential method to identify
}\textit{\DIFadd{cis}}\DIFadd{-regulatory }\DIFaddend regions. The \textit{Pkd2} example illustrates
how this could be done. Given that there is a \DIFdelbegin \DIFdel{cis-eQTL }\DIFdelend \DIFaddbegin \textit{\DIFadd{cis}}\DIFadd{-eQTL }\DIFaddend at this
locus \DIFaddbegin \DIFadd{(LOD = 144.8)}\DIFaddend , and that imputed chromatin state explained a large
amount of variance in DO gene expression, we looked more closely at
chromatin state within the inbred founders to identify putative
\DIFdelbegin \DIFdel{cis-regulatory }\DIFdelend \DIFaddbegin \textit{\DIFadd{cis}}\DIFadd{-regulatory }\DIFaddend regions (Fig. \ref{fig:example_gene}A arrows).
The variation in chromatin state across the inbred strains in these
regions is highly correlated with gene expression in the outbred
animals, pointing to possible molecular mechanisms for the observed
eQTL. The PWK and WSB haplotypes lack the activating states seen in the
other six haplotypes. This lack of activating states was correlated with
reduced gene expression in these strains. Further, the haplotypes with
broader activating states also appeared to have higher \textit{Pkd2}
expression suggesting that the width of the activating state may also
contribute to increased gene expression. The CpG sites in and arround
these putative regulatory regions are unmethylated across all strains,
further supporting the hypothesis \DIFdelbegin \DIFdel{chraomtin }\DIFdelend \DIFaddbegin \DIFadd{chromatin }\DIFaddend state in these regions is
...
gene \DIFdelbegin \DIFdel{is particularly interesting}\DIFdelend \DIFaddbegin \DIFadd{may also point to potentially novel regulatory mechanisms}\DIFaddend .
Variation in chromatin state at the intragenic enhancer is present in
the absence of local SNPs. This suggests that the presence of the
downstream enhancer is determined by another mechanism, perhaps SNPs
acting in \emph{trans} to this region, or local variation that was not
measured by SNP genotyping, e.g.~ indels. Genetic variation located at a
distance from the putative enhancer sites could also potentially alter
the 3D configuration of the genome resulting in variable access of
...
measured using \DIFdelbegin \DIFdel{RNA-Seq }\DIFdelend \DIFaddbegin \DIFadd{RNA-seq }\DIFaddend as described perviously
\citep{pmid27309819, pmid28592500}. Briefly, hepatocyte RNA was isolated
using the \DIFdelbegin \DIFdel{Trizol }\DIFdelend \DIFaddbegin \DIFadd{TRIzol }\DIFaddend Plus RNA extraction kit (Life Technologies), and 100-bp
...
The raw sequencing data from both \DIFdelbegin \DIFdel{RNA-Seq and ChIP-Seq }\DIFdelend \DIFaddbegin \DIFadd{RNA-seq and ChIP-seq }\DIFaddend were put through
...
\DIFdelbegin %DIFDELCMD < \hypertarget{chip-seq-quantification}{%
%DIFDELCMD < \subsubsection{ChIP-Seq quantification}\label{chip-seq-quantification}}
%DIFDELCMD < %%%
...
Code to run the analyses in this study are \DIFaddbegin \DIFadd{available }\DIFaddend at
\url{https://github.com/annaLtyler/Epigenetics_Manuscript} \DIFaddbegin \DIFadd{and in
Supplemental File XXX}\DIFaddend .
...
in each state \DIFdelbeginFL \DIFdelFL{tends }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{tended }\DIFaddendFL to be associated with expression in a manner
...
probabilities. Aligning the states across all models shows \DIFdelbeginFL \DIFdelFL{a remarkable 
stability in the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{that }\DIFaddendFL emissions
across models \DIFdelbeginFL \DIFdelFL{, seen }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{were stable. This stability is evident }\DIFaddendFL as vertical bars of 
consistent color.}
